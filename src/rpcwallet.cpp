...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid DigitalRupee address: ")+s.name_);
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("DigitalRupee-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("DigitalRupee-lock-wa");
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // DigitalRupee: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
// DigitalRupee: reserve balance from being staked for network protection
...
// DigitalRupee: check wallet integrity
...
// DigitalRupee: resend unconfirmed wallet transactions
...
// DigitalRupee: make a public-private key pair
